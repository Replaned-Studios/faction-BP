...
const number_of_1_16_100_items = 4;
const sizes = new Map([
...
class ChestFormData {
	...
	show(player, Way2 = null, FormRejectCallBack = null) {
		const form = new ActionFormData()
			.title(this.#titleText);
		    this.#buttonArray.forEach(button => {
			form.button(button[0], button[1]?.toString());
		})
		if (Way2) return form.show(player).then(({ cancelationReason, selection, canceled }) => {
			 this.callbacks.get(selection)?.(selection);
			 if (FormRejectCallBack) return FormRejectCallBack(cancelationReason, canceled)
